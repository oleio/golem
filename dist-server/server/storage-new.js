...
import { calculateFilamentStatus, applyStatusToFilaments, generateFilamentId, validateFilament } from './services/filament.service.js';
import { DEFAULT_SETTINGS, mergeWithDefaults, validateSettings } from './services/settings.service.js';
 ...
 * 读取所有耗材数据（包含状态计算）
 */
export async function readFilaments() {
    try {
 ...
 * 根据 ID 获取单个耗材
 */
export async function getFilamentById(id) {
    try {
 ...
 * 添加新耗材
 */
export async function addFilament(filament) {
    try {
        // 验证数据
 ...
 * 更新耗材
 */
export async function updateFilament(id, updates) {
    try {
        // 如果更新了重量，验证数据
        ...
        return {
            ...result,
            status: calculateFilamentStatus(result.weightRemaining, result.weightTotal, settings)
        };
    }
    catch (error) {
